...
inquiries_design <- readRDS("data/inquiries_without_answers_design.rds")
rd_design <-    readRDS("data/rd_design.rds")
srs_design <-   readRDS("data/srs_design.rds")
designs <- list("inquiries" = inquiries_design,
                "rd" = rd_design,
                "srs" = srs_design)

design_names <- list("inquiries" = "Inquiries without Answers",
                     "rd" = "Regression Discontinuity",
                     "srs" = "Simple Random Sample")

inquiries <- readRDS("data/inquiries_without_answers_diagnosis.rds")
rd <-    readRDS("data/rd_diagnosis.rds")
srs <-   readRDS("data/srs_diagnosis.rds")

sims <- inquiries$simulations %>% mutate(design = "inquiries") %>% sample_n(250) %>%
  bind_rows(rd$simulations %>% mutate(design = "rd") %>% sample_n(250)) %>%
  bind_rows(srs$simulations %>% mutate(design = "srs") %>% sample_n(250))


diagnosands <- inquiries$diagnosands %>% mutate(design = "inquiries") %>%
...
server <- function(input, output, session) {
  ...
  diagnosands_reactive <- reactive({
    print_diagnosands <- diagnosands %>% filter(design == design_name())
    print_diagnosands[, apply(print_diagnosands, 2, function(x) !all(is.na(x)))]
