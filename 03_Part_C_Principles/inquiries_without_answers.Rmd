...
Fortunately, when we ask a computer to answer such a question, it complains.
...
estimand_3 <- declare_estimand(ATE_Y_for_Ds = mean(Y_Z_1[D] - Y_Z_0[D]))
...
# Answer Strategy ---------------------------------------------------------
est_1 <- declare_estimator(R ~ Z, estimand = estimand_1, 
                           label = "DIM_R")
est_2 <- declare_estimator(Y ~ Z, estimand = list(estimand_2, estimand_3), 
                           label = "DIM_Y")

# Design ------------------------------------------------------------------
design <-   population / pos / 
...
```{r, echo=FALSE}
kable(reshape_diagnosis(diagnosis, col.names = "default")[,-2])
```
